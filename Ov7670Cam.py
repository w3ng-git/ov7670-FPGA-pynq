...
class Ov7670Cam:
    ...
    # 预设配置
    # ================== OV7670  VGA 640×480  RGB565 ==================
    CONFIG_NORMAL = [
        # ---------- 软件复位 ----------
        (0x12, 0x80), ("delay", 10),

        # ---------- 时钟 (24 MHz → PLL×4 = 96 MHz 内部像素时钟) ----------
        (0x11, 0x01),      # CLKRC: div = 1
        (0x6B, 0x4A),      # DBLV: enable PLL, ×4

        # ---------- RGB565 输出 ----------
        (0x12, 0x04),      # COM7: RGB | VGA
        (0x3A, 0x04),      # TSLB: 正常字节顺序
        (0x8C, 0x00),      # 关闭 RGB444
        (0x40, 0xD0),      # COM15: RGB565 & full-range 0–255

        # ---------- VGA 640×480 有效窗口 ----------
        (0x17, 0x13), (0x18, 0x01),   # HSTART / HSTOP
        (0x19, 0x02), (0x1A, 0x7A),   # VSTART / VSTOP
        (0x32, 0xB6),                 # HREF 边界
        (0x03, 0x0A),                 # VREF(低位)

        # ---------- 同步极性 / 方向 ----------
        (0x15, 0x00),   # COM10: 正极性 HS/VS
        (0x1E, 0x07),   # MVFP: 无镜像、无翻转
        ...
        # ---------- 自动增益 / 曝光 / 白平衡 ----------
        (0x13, 0xE7),  # COM8: AGC|AWB|AEC|BFILT|FASTAEC|AECSTEP
        (0x14, 0x48),  # COM9: 32× 增益上限
        (0x3D, 0xC2),  # COM13: Gamma | UV 饱和 | UV swap

        # 补丁 color patch---- below this line ------------------------------

        # ---------- “魔法”寄存器 ----------
        (0xB0, 0x84),      # undocumented – 打开改良色彩管线
